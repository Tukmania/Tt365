...
export default function MenWear() {
  ...
  useEffect(() => {
    async function fetchMenWearListings() {
      try {
        ...
        const q = query(
          menWearRef,
          where("category", "==", "Trending"), // Adjust the field and value based on your data model
          orderBy("timestamp"),
        );
  ...
  return (
    <div className="max-w-6xl mx-auto px-3">
      <h1 className="text-3xl text-center mt-6 font-bold mb-6">Trending Products</h1>
      {loading ? (
        <p>Loading...</p>
        ...
        <p>No trends wear products available.</p>
      )}
    </div>
  );
}
